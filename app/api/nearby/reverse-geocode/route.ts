...
import { nearbyReverseGeocodeRequestSchema } from "@/lib/schemas/nearby";
...
export async function POST(request: Request) {
  ...
  const parsed = nearbyReverseGeocodeRequestSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
      {
        error: {
  ...
  try {
    await consumePublicAmapQuota({
      ...
      units: publicAmapRequestUnits("reverse-geocode", parsed.data.coordinate_system),
    });
    const provider = getPlaceProvider(parsed.data.amap_web_service_key);
    const location = await provider.reverseGeocodeLocation({
      location: {
        latitude: parsed.data.latitude,
        longitude: parsed.data.longitude,
      },
      coordinateSystem: parsed.data.coordinate_system,
    });
    ...
    return NextResponse.json(
      { error: { code: "INTERNAL_ERROR", message: "详细地址解析失败，请稍后重试。" } },
      { status: 500 }
    );
  }
}
